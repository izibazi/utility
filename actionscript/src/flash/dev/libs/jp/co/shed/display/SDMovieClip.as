package jp.co.shed.display 
{
	 ...
	 */
	public class SDMovieClip extends MovieClip
	{
		 ...
		 * コンストラクタ
		 */
		public function SDMovieClip(...args) 
		{
			visible = false;
		 ...
		 * @param	target
		 */
		public function lock(target:MovieClip = null):void
		{
			if (target == null)
				target = this;
		 ...
		 * @param	target
		 */
		public function unlock(target:MovieClip = null):void
		{
			if (target == null)
				target = this;
...
/*		public function startBlink(target:MovieClip, add:Number = .1, middle:Number = .5):void
		{
			target.__middle = middle;
			target.__add = add;
			target.__angle = 0;
			target.addEventListener(Event.ENTER_FRAME, onBlink);
		}
		
		public function stopBlink(target:MovieClip):void
		{
			target.removeEventListener(Event.ENTER_FRAME, onBlink);
		}
		
		private function onBlink(e:Event):void
		{
			var target:MovieClip = e.target as MovieClip;
			target.__angle += target.__add;
			bright(target , NaN, Math.cos(target.__angle) * target.__middle + target.__middle, 0);
			target.__angle %= (Math.PI * 2);
		}*/

		/**
		 * ムービークリップ型にキャストしたオブジェクトを返却します。
		 */
		public function get asMovieClip():MovieClip
		{
		 ...
		 * @return
		 */
		public function tween(p_scopes:Object, time:Number = .2, delay:Number = 0, p_parameters:Object = null):Boolean 
		{
		 ...
		 * @return
		 */
		public function tweenParam(p_scopes:Object, param:String, to:Number, time:Number = .2, delay:Number = 0, transition:String = "", p_parameters:Object = null):Boolean 
		{
		 ...
		 * @return
		 */
		public function position(p_scopes:Object, fromX:Number = NaN, fromY:Number = NaN, toX:Number = 0, toY:Number = 0, time:Number = .2, delay:Number = 0, transition:String = "ealseoutexpo", p_parameters:Object = null ):Boolean 
		{
